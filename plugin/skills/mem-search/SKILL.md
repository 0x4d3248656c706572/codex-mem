...
### User-Installable Grammars via `.codex-mem.json`

Add custom tree-sitter grammars for languages not in the bundled set. Place `.codex-mem.json` in the project root:
...
```json
{
  "grammars": {
    ...
    "protobuf": {
      ...
      "extensions": [".proto"],
      "query": ".codex-mem/queries/proto.scm"
    }
  }
}
```
...
- Config is cached per project root. Changes to `.codex-mem.json` take effect on next worker restart.
