 ...
 *
 * Resolves the install directory from CLAUDE_PLUGIN_ROOT (set by Codex
...
// Early exit if plugin is disabled in Codex settings (#781)
 ...
 *
 * Priority:
 * 1. CLAUDE_PLUGIN_ROOT env var (set by Codex for hooks — works for
...
function resolveRoot() {
  // CLAUDE_PLUGIN_ROOT is the authoritative location set by Codex
...
function installDeps() {
  ...
  // Use pipe for stdout to prevent non-JSON output leaking to Codex hooks.
...
try {
  ...
  // Output valid JSON for Codex hook contract
  console.log(JSON.stringify({ continue: true, suppressOutput: true }));
} catch (e) {
  console.error('❌ Installation failed:', e.message);
  // Still output valid JSON so Codex doesn't show a confusing error
  console.log(JSON.stringify({ continue: true, suppressOutput: true }));
  process.exit(1);
}
